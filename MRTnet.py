...
class MRTnet_lstm(nn.Module):
    def __init__(self, backbone, grad_norm=False, out_channel=1024, dropout=0.3, word_size=1024, use_crf=False):
        super(MRTnet_lstm, self).__init__()
    ...
    def forward(self, x, sequence_length, labels=None):
        ...
        feature = torch.cat((feature_i, feature_v, feature_t), 1)  # batch_size * 3 * 1024

        label_embedding = self.label_embedding(torch.arange(0, 31).repeat((batch_size, 1)).to(feature.device))
        feature = self.transformer_decoder_ivt(feature.transpose(0, 1),
                                                       label_embedding.transpose(0, 1)).transpose(0,
                                                                                                  1)  # batch_size * 3 * 1024
        feature_i = feature[:, 0, :]
        feature_v = feature[:, 1, :]
        feature_t = feature[:, 2, :]
        cat_feature = feature.contiguous().view(int(batch_size / sequence_length), sequence_length, -1)
    ...
    def get_optim_policies(self, backbone_lr, subnet_lr, crf_lr):
        ...
        if self.grad_norm:
            param.append({'params': self.weights, 'lr': backbone_lr, 'name': 'loss_weight'})
        return param
    ...
    def get_backbone_last_layer(self):
        return self.conv5[-1]